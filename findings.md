...
|---|---|---|---|
| IfcOpenShell | 0.10-0.25s | 0.65-2.11s | 3.84-22.88s |
...
**Parsing**: web-ifc is consistently 5-10x faster. For the 303MB file: 2.37s vs 22.88s.

**Geometry**: web-ifc is generally 1.5-4x faster despite being single-threaded WASM vs IfcOpenShell's multiprocessed CGAL+OpenCASCADE. Notable exceptions:
- private4 (303MB): web-ifc 135s vs IfcOpenShell 61s — web-ifc hits something pathological
- ISSUE_098 (68MB): IfcOpenShell 7.4s vs web-ifc 10.0s
- private1 (212MB): web-ifc 2.7s vs IfcOpenShell 41.2s — IfcOpenShell's CGAL/OCC struggles with this model

**Product counts**: Nearly identical. S_Office has a small 10-product discrepancy (3417 vs 3407).

**Takeaways**: For a pipeline that needs fast open + iterate, web-ifc currently wins overall. IfcOpenShell's multiprocessing doesn't compensate for its heavier geometry kernel on most models. Geometry complexity matters more than file size — both engines have individual models where they choke.
